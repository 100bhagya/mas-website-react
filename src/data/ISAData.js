const Data = [
  {
    question: "What is PAP and how does it work?",
    answer:
      "PAP (Pay After Placement) is basically a model of investment which students have to sign at the time of registration and they become entitled to pay a small amount of their salary once the job starts.",
    open: true,
  },
  {
    question: "When do my PAP repayments start?",
    answer:
      "Your PAP repayments start as soon as you start getting your first salary.",
    open: false,
  },
  {
    question: "What is the true cost of PAP?",
    answer:
      "To know the true cost of PAP please refer above 'How Does PAP Work' section.",
    open: false,
  },
  {
    question: "Is there an alternative method of payment apart from PAP?",
    answer:
      "There are multiple payment options available. For more information regarding payment structures, please refer above Salary slabs and their respective PAP payment.",
    open: false,
  },
  {
    question: "Do I need to submit any document after getting placed?",
    answer:
      "Yes, you need to submit the offer letter and other relevant documents to ensure a smooth PAP transactions.",
    open: false,
  },
  {
    question: "What happens in case I leave my job or get fired in between?",
    answer:
      "In case you are fired/you leave the job/you switch the job before completion of PAP, your PAP will be paused and will resume with your next job till completion. (Not applicable in case you don't wish to do a job further).",
    open: false,
  },
];

export default Data;
